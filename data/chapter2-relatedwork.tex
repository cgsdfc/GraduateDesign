...
如图~\ref{fig:RNN_unrolled}
\footnote{\url{http://colah.github.io/posts/2015-08-Understanding-LSTMs/}}~所示，
...
这些指标的基础是信息检索领域常用的F得分（F-measure），即准确率和召回率的加权调和平均值：
\begin{align}
...
提出了自动化对话评价模型（Automatic Dialogue Evaluation Model，ADEM）\upcite{ADEM}。
...
% ------ RUBER --------- %
Tao等人提出了Referenced Metric and Unreferenced Metric Blended Evaluation Routine\upcite{RUBER}，简称RUBER。
...
该指标在中文数据集豆瓣网\footnote{\url{http://www.douban.com}}上取得了很高的人类评价相关性。
\begin{figure}[H]
...
Li等人在\upcite{Adversarial}中构建了完整的对抗生成网络，
提出了训练和对抗式评价的一体框架。
由于篇幅有限，不再描述。
...
\upcite{GoogleChatbot}的评估使用了困惑度，定性分析和人类评价。
...
如时间戳，对话双方身份信息等等。
数据集\upcite{DailyDialog,LSDSCC,DCGM}有人类标注信息，
如情感、话题和方面（Aspect）等。
元信息和人类标注信息提供了额外的表征，对构建更智能的对话系统很有帮助。
本文把注意力集中在三个数据集上：
Ubuntu对话数据集，OpenSubtitles，LSDSCC，
...
庞大而充满噪音的开放领域数据集。
它的对话数量达到了80M。
由于OPUS的目的是收集机器翻译所用到的双语数据集（Parallel Corpus），OpenSubtitles也是一个双语数据集，
它里面并没有对话数据集所需要的轮换信息（Turn-Taking），
而且也没有区分旁白、独白和对白。
Sordoni等人在\upcite{GoogleChatbot}中把OpenSubtitles用到对话领域，
他们把相邻的两个句子视为消息和响应，并且每一个句子既是消息又是响应。
Li等人在\upcite{MMI,persona,Future,DiverseBeam,
Distill,deep_RL,Adversarial}中也采用了类似的办法。
尽管充满噪音，OpenSubtitles仍然是目前最大规模的电影字幕数据集。

% --------- LSDSCC ------------- %
LSDSCC，A Large Scale Domain-Specific Conversational Corpus，是一个单一领域的单轮对话数据集\upcite{LSDSCC}。
...
最终得到一个高质量的信息-响应对（Query-Response Pair）数据集。
在构建测试集方面，作者用类似\upcite{DCGM}的IR方法构建了一个多重参考评测集。
...
因为数量的增多，一些数据集在领域上出现了重合，
比如电影对话数据集（Movie Dialogs Corpus），
电影三元组数据集（MovieTriples）和Movie-DiC的领域都是电影对白；
OpenSubtitles和SubTle的领域都是电影字幕；
Twitter数据集（Twitter Corpus），Twitter对话数据集和Twitter三元组数据集的领域都是Twitter短文本闲聊。
% 新浪微博数据集（Sina Weibo Corpus）
