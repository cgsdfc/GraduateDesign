...
另一方面，生成式对话系统在不同数据集上的迁移能力似乎是一个研究的比较少的问题。
开放领域的对话数据集所具有的大量噪音，
...
和相对有限的数据集和指标相比，生成式模型的发展呈现出日新月异的态势，
光是我们了解到的不同的模型，就有不下十几个。
然而我们的时间和精力是有限的，只能根据实验的需求选取几个有代表性的模型进行实验，
这是一个无可奈何的选择。
...
\subsection{多层编解码器}\label{subsec:HRED}
